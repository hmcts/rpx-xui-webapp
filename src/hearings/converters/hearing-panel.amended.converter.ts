...
export class HearingPanelAmendedConverter implements IsAmendedConverter {
  public transformIsAmended(hearingState$?: Observable<State>): Observable<boolean> {
    return hearingState$.pipe(map((state) => {
      ...
      const comparedPanelMembers: number = hearingComparePanelRequirements && hearingComparePanelRequirements.panelPreferences && hearingComparePanelRequirements.panelPreferences.filter((preferences) => preferences.memberType === MemberType.PANEL_MEMBER).length || 0;
      const comparedPanelSpecialisms: number = hearingComparePanelRequirements && hearingComparePanelRequirements.panelSpecialisms && hearingComparePanelRequirements.panelSpecialisms.length || 0;
