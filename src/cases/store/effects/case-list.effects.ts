...
export class CaseListEffects {
    ...
    this.actions$.pipe(
      ...
      switchMap((payload) => {
        ...
        return this.searchService.findPaginationMetadata(payload).pipe(
          map((response) => new caselistActions.FindCaselistPaginationMetadataSuccess(response.json())),
  ...
  public applyCaselistFiltersForES$ = createEffect(() =>
    this.actions$.pipe(
  ...
  public applySearchFilterToggle$ = createEffect(() =>
    this.actions$.pipe(
