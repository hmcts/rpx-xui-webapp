...
class CaseListPage{
  ...
  async caseDataValidation(){
    ...
    for (const casevalue in caseValues){
      if (caseFields[casevalue]){
        const thObj = cases.columns.find((caseObj) => caseObj.label == caseFields[casevalue]);

        const thKey = thObj.case_field_id;

        const value = cases.results[0].case_fields[thKey] ? cases.results[0].case_fields[thKey] : '';
  ...
  async validateCaseValues() {
    await BrowserWaits.waitForElement(this.caseResultsPagination);
    const caseListTd = element.all(by.xpath('//table/tbody/tr[1]/td'));
    const tdCount = await caseListTd.count();
    // let caseListTd = element.all(by.xpath(`//table/tbody/tr[1]/td`));
    await BrowserWaits.waitForElement(this.caseListTableHead);
    console.log('tdCount', tdCount);
    const caseData = [];
    for (let i = 0; i < tdCount; i++) {
      const thText = caseListTd.get(i).$$('ccd-field-read-label');
      const text = await thText.getText();
      const updatedText =await text.toString().replace(/-/g, '');
