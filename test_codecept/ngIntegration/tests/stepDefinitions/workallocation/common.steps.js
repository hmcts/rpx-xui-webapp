...
When(
  'I click task list pagination link {string} and wait for req reference {string} not null',
  async function (paginationLinktext, reference) {
...
When(
  'I click WA case list pagination link {string} and wait for req reference {string} not null',
  async function (paginationLinktext, reference) {
    await waCaseListTable.waitForTable();
    await BrowserWaits.retryWithActionCallback(async () => {
      const val = await browserUtil.addTextToElementWithCssSelector(
        'tbody tr .cdk-column-case_category exui-task-field,tbody tr .cdk-column-case_category exui-work-field',
        'Sort test',
        true
      );
      if (val !== 'success') {
...
Then(
  'I validate task search request with reference {string} has pagination parameters',
  async function (requestReference, datatable) {
...
Then(
  'I validate task search request with reference {string} have search parameters',
  async function (requestReference, datatable) {
    ...
    for (let i = 0; i < datatableHash.length; i++) {
      ...
      if (Object.keys(TASK_SEARHC_FILTERS_TO_IGNORE_IN_REQUEST_BODY).includes(searchHash.key)) {
        CucumberReporter.AddMessage(
          `${searchHash.key} is ignored for eeason : ${TASK_SEARHC_FILTERS_TO_IGNORE_IN_REQUEST_BODY[searchHash.key]}`
        );
        continue;
      }
...
Then(
  'I validate task search request with reference {string} does not have search parameters',
  async function (requestReference, datatable) {
    const softAssert = new SoftAssert();
    const reqBody = global.scenarioData[requestReference];
    const datatableHash = datatable.parse().hashes();
...
Then(
  'I validate task search request with reference {string} does not have search patameter key {string}',
  async function (requestReference, searchKey) {
    const reqBody = global.scenarioData[requestReference];

    const reqSearchParams = reqBody.searchRequest.search_parameters;
    const searchParametersMatchingType = await ArrayUtil.filter(
      reqSearchParams,
      async (searchObj) => searchObj.key === searchKey
    );

    expect(
      searchParametersMatchingType.length,
      `Search parameter mathcing key "${searchKey}" found in request body ${reqBody} `
    ).to.equal(0);
  }
);
...
Then(
  'If current user {string} is {string}, I validate task table values displayed',
  async function (currentUser, validationForUserType, datatable) {
  ...
  for (let i = 0; i < tableRowHashes.length; i++) {
    ...
    for (let j = 0; j < hashkeys.length; j++) {
      ...
      await softAssert.assert(async () =>
        expect(actualColumnValue.toLowerCase(), assertionMessage).to.includes(expectedValue.toLowerCase())
      );
    }
  }
  softAssert.finally();
