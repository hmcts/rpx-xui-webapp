...
export class WorkCaseListWrapperComponent implements OnInit {
  ...
  public onActionHandler(caseAction: InvokedCaseAction): void {
    ...
    if (thisAction.id === Actions.Reallocate) {
      ...
      actionUrl = `role-access/allocate-role/${thisAction.id}?caseId=${actionedCase.case_id}&assignmentId=${actionedCase.id}&roleCategory=${actionedCase.case_category}&jurisdiction=${actionedCase.jurisdiction}`;
    }
