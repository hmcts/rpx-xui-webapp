...
export class DecentralisedEventGuard implements CanActivate, CanActivateChild {
  ...
  canActivate(route: ActivatedRouteSnapshot): MaybeAsync<GuardResult> {
    ...
    if (this.decentralisedRedirectService.isDecentralisedEvent(eventId)) {
      const caseType = route.paramMap.get('ctid') ?? route.paramMap.get('caseType');
      ...
      if (caseType) {
        ...
        let event;
        if (caseId) {
          event = DecentralisedEvent.forCase(eventId, caseType, caseId, route.queryParams);
        } else if (jurisdiction) {
          event = DecentralisedEvent.forCreateCase(eventId, caseType, jurisdiction, route.queryParams);
        }

        if (event) {
          return this.decentralisedRedirectService.tryRedirectEvent(event);
        }
      }
    }
