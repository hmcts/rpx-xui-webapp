  ...
  public transformAnswer(hearingState$: Observable<State>): Observable<string> {
    return hearingState$.pipe(
      map(state => {
        ...
        if (hearingWindow && hearingWindow.hearingWindowDateRange && hearingWindow.hearingWindowDateRange.hearingWindowStartDateRange && hearingWindow.hearingWindowDateRange.hearingWindowEndDateRange) {
          specificDateSelection = RadioOptions.CHOOSE_DATE_RANGE;
          const earliestHearing = moment(hearingWindow.hearingWindowDateRange.hearingWindowStartDateRange).format(HearingDateEnum.DisplayMonth);
          const latestHearing = moment(hearingWindow.hearingWindowDateRange.hearingWindowEndDateRange).format(HearingDateEnum.DisplayMonth);
        ...
        } else if (hearingWindow && hearingWindow.hearingWindowDateRange && hearingWindow.hearingWindowDateRange.hearingWindowStartDateRange && !hearingWindow.hearingWindowDateRange.hearingWindowEndDateRange) {
          specificDateSelection = RadioOptions.YES;
          const firstDate = moment(hearingWindow.hearingWindowDateRange.hearingWindowStartDateRange).format(HearingDateEnum.DisplayMonth);
