...
export async function selectOptionWithRetry(
  ...
  const { attempts = 6, delayMs = 5000, label = 'Jurisdiction' } = opts;
...
export async function expectTextVisibleWithRetry(page: Page, text: string, opts: VisibleRetryOpts = {}) {
  const { exact = true, attempts = 6, delayMs = 2000, timeoutPerAttempt = 5000 } = opts;
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      await expect(page.getByText(text, { exact })).toBeVisible({ timeout: timeoutPerAttempt });
      return;
    } catch (err) {
      if (i === attempts - 1) {
        throw err;
