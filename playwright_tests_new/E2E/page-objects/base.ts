...
export abstract class Base {
  ...
  constructor(public readonly page: Page) {
    if (!Base.pageApiCalls.has(page)) {
      Base.pageApiCalls.set(page, []);
    }
  ...
  private getTrackedApiCalls(): ApiCall[] {
    const existingCalls = Base.pageApiCalls.get(this.page);
    if (existingCalls) {
      return existingCalls;
    }

    const newCalls: ApiCall[] = [];
    Base.pageApiCalls.set(this.page, newCalls);
    return newCalls;
  }

  private recordApiCall(call: ApiCall): void {
    const apiCalls = this.getTrackedApiCalls();
    apiCalls.push(call);

    // Prevent memory leaks: cap at maxApiCallsTracked (FIFO - remove oldest)
    if (apiCalls.length > this.maxApiCallsTracked) {
      apiCalls.shift(); // Remove oldest call
    }
  }
  ...
  public getApiCalls(): ApiCall[] {
    return [...this.getTrackedApiCalls()];
  }

  public clearApiCalls(): void {
    this.getTrackedApiCalls().length = 0;
  }
...
`;
  ...
  protected getApiTimingStats(sampleSize = 50): { count: number; avg: number; p95: number } {
    const durations = this.getTrackedApiCalls()
      .slice(-sampleSize)
      .map((c) => c.duration)
