    ...
    for (const party of parties) {
      ...
      const relatedParty = {
        ...
        individualDetails: {
          firstName: party.individualDetails?.firstName
            ? `Related ${party.individualDetails.firstName}`
            : 'Realted party First Applicant FN updated',
          lastName: party.individualDetails?.lastName
            ? `Related ${party.individualDetails.lastName}`
            : 'Related party First Applicant LN updated',
      ...
      const partyIndividualDetails = {
        firstName: party.individualDetails?.firstName
          ...
          : `${party.partyName ? party.partyName + ' FN' : 'First app FN'}`,
        lastName: party.individualDetails?.lastName
          ? party.individualDetails.lastName
          : `${party.partyName ? party.partyName + ' LN' : 'First app LN'}`,
        interpreterLanguage: party.individualDetails?.interpreterLanguage ? party.individualDetails.interpreterLanguage : '',
        reasonableAdjustments: party.individualDetails?.reasonableAdjustments
          ? party.individualDetails.reasonableAdjustments.split(',')
          : [],
        vulnerableFlag: false,
        vulnerabilityDetails: '',
        hearingChannelEmail: ['applicant@test.com'],
        hearingChannelPhone: ['1234567890'],
