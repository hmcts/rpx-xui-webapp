...
import { expectWaveLikeAccessibility } from '../../utils/accessibility/waveLikeAccessibility';
import {
...
test.describe('authenticated webapp WAVE-like accessibility @wave-a11y', () => {
  ...
  test('task list page passes WAVE-like structural checks', async ({ taskListPage, page }) => {
    await taskListPage.goto();
    await expect(taskListPage.taskListTable).toBeVisible();
    await expectWaveLikeAccessibility(page);
  });

  test('case list page passes WAVE-like structural checks', async ({ caseListPage, page }) => {
    await caseListPage.goto();
    await expect(caseListPage.container).toBeVisible();
    await expectWaveLikeAccessibility(page);
  });

  test('case search page passes WAVE-like structural checks', async ({ page, caseListPage }) => {
    await page.goto('/cases/case-search');
    await expect(caseListPage.filtersContainer).toBeVisible();
    await expectWaveLikeAccessibility(page);
  });

  test('case share page passes WAVE-like structural checks', async ({ page }) => {
    await page.goto('/cases/case-share?init=true');
    await expect(page.locator('main')).toBeVisible();
    await expectWaveLikeAccessibility(page);
  });

  test('case share validation state passes WAVE-like structural checks', async ({ page }) => {
  ...
  test('create case filter page passes WAVE-like structural checks', async ({ page }) => {
    await page.goto('/cases/case-filter');
    await expect(page.locator('main')).toBeVisible();
    await expectWaveLikeAccessibility(page);
  });

  test('global find case shell passes WAVE-like structural checks', async ({ page, searchCasePage }) => {
  ...
  test('find case no-results state passes WAVE-like structural checks', async ({ page, searchCasePage }) => {
    await page.goto('/cases');
    await expect(searchCasePage.caseIdTextBox).toBeVisible();
  ...
  test('specific access request validation state passes WAVE-like structural checks', async ({ accessRequestPage, page }) => {
  ...
  test('challenged access request validation state passes WAVE-like structural checks', async ({ accessRequestPage, page }) => {
  ...
  test('review specific access service-down state passes WAVE-like structural checks', async ({ accessRequestPage, page }) => {
  ...
  test('role access delete exclusion page passes WAVE-like structural checks', async ({ page }) => {
    await page.goto('/role-access/delete-exclusion?caseId=1620409659381330&exclusionId=123');
    await expect(page.locator('main')).toBeVisible();
    await expectWaveLikeAccessibility(page);
  });

  test('role access allocate route shell passes WAVE-like structural checks', async ({ page }) => {
    await page.goto(
  ...
  test('all work tasks page passes WAVE-like structural checks', async ({ taskListPage, page }) => {
    await taskListPage.gotoAllWorkTasks();
    await expect(taskListPage.taskListTable).toBeVisible();
    await expectWaveLikeAccessibility(page);
  });

  test('all work cases page passes WAVE-like structural checks', async ({ taskListPage, page }) => {
    await taskListPage.gotoAllWorkCases();
    await expect(page.locator('main')).toBeVisible();
    await expectWaveLikeAccessibility(page);
  });

  test('my cases page passes WAVE-like structural checks', async ({ taskListPage, page }) => {
    await taskListPage.gotoMyCases();
    await expect(page.locator('main')).toBeVisible();
    await expectWaveLikeAccessibility(page);
  });

  test('my access page passes WAVE-like structural checks', async ({ taskListPage, page }) => {
  ...
  for (const staticPage of staticAndErrorPages) {
    test(`${staticPage.name} static/error page passes WAVE-like structural checks`, async ({ page }) => {
