...
test.describe(`Case List as ${userIdentifier}`, { tag: ['@integration', '@integration-case-list'] }, () => {
  ...
  await test.step('Verify user can see a list of cases less than page size given the mock response', async () => {
    ...
    await expect(caseListPage.caseListResultsAmount).toHaveText(
      `Showing 1 to ${expectedCount} of ${mock.total} results`
    );
  ...
  test(`User ${userIdentifier} can view more than 100 cases on the case list page`, async ({ caseListPage, tableUtils, page }) => {
    ...
    await test.step('Verify user can see a list of cases more than 100 based given the mock response', async () => {
      ...
      //await caseListPage.clickPaginationPage(5);
      //await expect(caseListPage.paginationCurrentPage).toContainText('5');
      // expect(caseListPage.paginationPrevious).toBeVisible();
      //wait expect(caseListPage.paginationNext).not.toBeVisible();
