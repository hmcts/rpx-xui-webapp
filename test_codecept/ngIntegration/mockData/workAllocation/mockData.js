...
class WorkAllocationMockData {
  ...
  setCasesWithPermissionsForView(viewInTest, casePermissionHashes){
    ...
    for (let i = 0; i < casePermissionHashes.length; i++) {
      const caseCount = casePermissionHashes[i].Count;
  ...
  async getFindPersonsDataFrom(database) {
    return await ArrayUtil.map(database, async (personDetails) => {
      const personModel = WorkAllocationDataModels.getFindPersonObj();
      const personDetailsKeys = Object.keys(personDetails);
  ...
  getMyWorkMyTasks(count) {
    const tasks = { tasks: [], total_records: count };
    for (let i = 0; i < count; i++) {
  ...
  getWACases(count) {
    const casesResponse = { cases: [], total_records: count };
    for (let i = 0; i < count; i++) {
  ...
  getRelease1TaskList(count, actions) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const task = WorkAllocationDataModels.getRelease1Task();
      task.id = v4();
      task.assignee = v4();
  ...
  async getCaseTasks(tasksObjects, userDetails) {
    ...
    const tasks = [];
    for (const task of tasksObjects) {
