export enum AllocationTitleText {
  Exclusion = 'Choose who the exclusion is for',
  Judiciary = 'Choose who to allocate the role to',
  LegalOps = 'Choose how to allocate the role'
}

export enum AllocationCaptionText {
  Exclusion = 'Add an exclusion',
  Judiciary = 'Allocate a hearing judge',
  LegalOps = 'Allocate a legal ops case manager'
}

export enum AllocationRadioText {
  NonExclusionSelf = 'Reserve to me',
  NonExclusionOther = 'Allocate to another person',
  ExclusionSelf = 'Exclude me',
  ExclusionOther = 'Exclude another person'
}
