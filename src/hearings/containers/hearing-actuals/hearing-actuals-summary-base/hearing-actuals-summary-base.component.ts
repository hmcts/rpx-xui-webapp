...
import { ACTION, AnswerSource, HearingChannelEnum, PartyType } from '../../../models/hearings.enum';
...
export class HearingActualsSummaryBaseComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    ...
    this.sub = this.hearingState$.pipe(filter((state) => !!state.hearingActuals.hearingActualsMainModel)).subscribe((state) => {
      ...
      this.hearingOutcome =
        this.hearingActualsMainModel.hearingActuals && this.hearingActualsMainModel.hearingActuals.hearingOutcome;
      this.hearingTypeDescription =
        this.hearingOutcome?.hearingType && this.getHearingTypeDescription(this.hearingOutcome.hearingType);
      ...
      this.individualParties = state.hearingValues.serviceHearingValuesModel?.parties?.filter(
        (party) => party.partyType === PartyType.IND
      );

      this.hearingActualsMainModel.hearingPlanned.plannedHearingDays.forEach((plannedDay) => {
  ...
  public getHearingTypeDescription(hearingType: string): string {
    const hearingTypeFromLookup =
      this.hearingTypes && this.hearingTypes.find((x) => x.key.toLowerCase() === hearingType.toLowerCase());
  ...
  public calculateEarliestHearingDate(hearingDays: ActualHearingDayModel[]): string {
    const moments: moment.Moment[] = hearingDays.map((d) =>
      moment.tz(this.convertUTCDateToLocalDate(new Date(d.hearingStartTime)), moment.tz.guess())
    );
    if (moments.length > 1) {
  ...
  public isDetailsProvidedForDay(day): boolean {
    if (
      ...
      const actualDay = this.hearingActualsMainModel.hearingActuals.actualHearingDays.find(
        (d) => Date.parse(d.hearingDate) === Date.parse(day.hearingDate)
      );
      if (actualDay) {
        const hasActualTiming = Boolean(
          actualDay.hearingDate && actualDay.hearingStartTime && actualDay.hearingEndTime && actualDay.pauseDateTimes
        );
