...
Given('I set MOCK locations for WA release 2', async function (locationsDatatable) {
  ...
  for (let i = 0; i < locationsHashes.length; i++) {
    ...
    const locationHashKeys = Object.keys(locationsHashes[i]);
...
Given('I set MOCK persons end point {string} for WA release 2', async function (endpoint, personsDatatable) {
  ...
  for (let i = 0; i < personshashes.length; i++) {
    ...
    const personHashKeys = Object.keys(personshashes[i]);
...
Given('I set MOCK judicial users end point {string} for WA release 2', async function (endpoint, personsDatatable) {
  ...
  const personsResponseBody = [];
  for (let i = 0; i < personshashes.length; i++) {
    const person = workAllocationDataModel.getRefDataJudge();
    personsResponseBody.push(person);
    const personHashKeys = Object.keys(personshashes[i]);
    personHashKeys.forEach((key) => {
      person[key] = personshashes[i][key];
...
Given(
  'I set MOCK tasks with permissions for view {string} and assigned state {string}',
  async function (inputView, assignedState, taskPermissionsTable) {
    ...
    switch (view) {
      case 'mytasks':
        ...
        await mockClient.setUserApiData(auth.value, taskManagementApiMock.method.searchTasks, {
          status: 200,
          data: workAllocationMockData.myWorkMyTasks,
        });
        break;
      case 'availabletasks':
        ...
        await mockClient.setUserApiData(auth.value, taskManagementApiMock.method.searchTasks, {
          status: 200,
          data: workAllocationMockData.myWorkAvailableTasks,
        });
      ...
      case 'allwork':
        ...
        await mockClient.setUserApiData(auth.value, taskManagementApiMock.method.searchTasks, {
          status: 200,
          data: workAllocationMockData.allWorkTasks,
        });
...
Given('I set MOCK tasks with attributes for view {string}', async function (forView, attributesDatatable) {
  ...
  for (let i = 0; i < tasksHashes.length; i++) {
    ...
    const task = tasksResponse.tasks[i];

    const taskHashKeys = Object.keys(taskHash);
  ...
  const auth = await browser.driver.manage().getCookie('__auth__');
  await backendMockClient.setUserApiData(auth.value, taskManagementApiMock.method.searchTasks, {
    status: 200,
    data: tasksResponse,
  });
});
...
Then(
  'I validate {string} tasks columns sorting with taskRequest url {string} on page {int} for user type {string}',
  async function (waPage, taskRequesturl, onPage, userType, datatable) {
    ...
    for (let i = 0; i < datatableHashes.length; i++) {
      ...
      await browserUtil.addTextToElementWithCssSelector(
        'tbody tr:nth-of-type(1) .cdk-column-case_category exui-work-field',
        'Sort test',
        true
      );
      ...
      await softAssert.assert(
        async () => expect(asc_sortingParamsInReq.length > 0).to.be.true,
        'Sorting param present in request'
      );
      await softAssert.assert(
        async () => expect(sortColumnInRequestParam.searchRequest.pagination_parameters.page_number).to.equal(onPage),
        'Sorting req on page'
      );
      await softAssert.assert(
        async () => expect(asc_sortingParamsInReq[0].sort_by).to.equal(sortColumnId),
        'Sorting param name in request ' + sortColumnId
      );
      await softAssert.assert(
        async () => expect(asc_sortingParamsInReq[0].sort_order).to.equal('asc'),
        'Sorting param value in request asc'
      );
      await softAssert.assert(
        async () => expect(await pageUndertest.getColumnSortState(headerName)).to.equal('ascending'),
        'Sort column state ascending'
      );
      ...
      await softAssert.assert(
        async () => expect(descSortingParamsInReq.length > 0).to.be.true,
        'Sorting param present in request'
      );
      await softAssert.assert(
        async () => expect(sortColumnInRequestParam.searchRequest.pagination_parameters.page_number).to.equal(onPage),
      ...
      await softAssert.assert(
        async () => expect(descSortingParamsInReq[0].sort_by).to.equal(sortColumnId),
        'Sorting param name in request ' + sortColumnId
      );
      await softAssert.assert(
        async () => expect(descSortingParamsInReq[0].sort_order).to.equal('desc'),
        'Sorting param value in request desc'
      );
      await softAssert.assert(
        async () => expect(await pageUndertest.getColumnSortState(headerName)).to.equal('descending'),
        'Sort column state descending'
      );
    }
    softAssert.finally();
  }
...
Then(
  'I validate work filter get location request body {string}, user locations for service',
  async function (requestBodyref, datatable) {
    const body = global.scenarioData[requestBodyref];
    CucumberReporter.AddJson(body);
    ...
    for (const expectedLocationsForService of userLocationsExpected) {
      const actualServiceLocations = userLocationsActual.find(
        (actulaLocationsForService) => actulaLocationsForService.service === expectedLocationsForService.service
      );

      if (expectedLocationsForService.locations === '') {
        expect(actualServiceLocations === undefined, `Locations for service object ${expectedLocationsForService.service}   sent`)
          .to.be.true;
      } else {
        expect(actualServiceLocations !== undefined, `Locations for service ${expectedLocationsForService.service}  not sent`).to
          .be.true;
        ...
        for (const loc of expectedLocations) {
          expect(
            actualLocationIds,
            `user ${expectedLocationsForService.service} service location is not present in request body`
          ).to.includes(loc);
        }
      }
    }
