...
test.describe(" Shamrez CASE HISTORY  ", () => {
  ...
  test("Shamrez CASE HISTORY -- Verify creating a case works as expected @SD", async ({ validatorUtils, createCasePage, caseListPage, tableUtils }) => {
    let caseNumber: string;
    let textField0 = faker.lorem.word();

    console.log('Inside of Shamrez CASE HISTORY ') ;

    await test.step("Create a case and validate the case number @SD", async () => {
    ...
    await test.step("Shamrez Find the created case in the case list @SD", async () => {
