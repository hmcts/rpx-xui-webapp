...
function getValueByPropertyName(selectCase: SearchResultViewItem, propName: string): any {
  return selectCase.case_fields && selectCase.case_fields.hasOwnProperty(propName) ? selectCase.case_fields[propName] : '';
}
...
function combineCaseTitleByCaseType(caseTypeId: string, selectCase: SearchResultViewItem): string {
  if (caseTypeId.includes('FinancialRemedy')) {
    ...
    const marriagePetitionerName =
      getValueByPropertyName(selectCase, 'D8PetitionerFirstName') +
      BLANK_SPACE +
      getValueByPropertyName(selectCase, 'D8PetitionerLastName');
    const marriageRespondentName =
      getValueByPropertyName(selectCase, 'D8RespondentFirstName') +
      BLANK_SPACE +
      getValueByPropertyName(selectCase, 'D8RespondentLastName');
...
function getApplicantName(selectCase: SearchResultViewItem) {
  return getValueByPropertyName(selectCase, 'applicantFMName') && getValueByPropertyName(selectCase, 'applicantLName')
    ...
    : getValueByPropertyName(selectCase, 'applicantLName')
      ? getValueByPropertyName(selectCase, 'applicantLName')
      : EMPTY_SPACE;
}
...
function getRespondentName(selectCase: SearchResultViewItem) {
  let respondentName =
    getValueByPropertyName(selectCase, 'appRespondentFMName') && getValueByPropertyName(selectCase, 'appRespondentLName')
      ? getValueByPropertyName(selectCase, 'appRespondentFMName') +
        BLANK_SPACE +
  ...
  if (!respondentName) {
    respondentName =
      getValueByPropertyName(selectCase, 'respondentFMName') && getValueByPropertyName(selectCase, 'respondentLName')
        ? getValueByPropertyName(selectCase, 'respondentFMName') +
          BLANK_SPACE +
          getValueByPropertyName(selectCase, 'respondentLName')
        : getValueByPropertyName(selectCase, 'respondentLName')
          ? getValueByPropertyName(selectCase, 'respondentLName')
          : EMPTY_SPACE;
  }
