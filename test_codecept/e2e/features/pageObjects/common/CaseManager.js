...
class CaseManager {
  ...
  async submitCase(isAccessibilityTest){
    const checkYouranswers = $('.check-your-answers');
    const isCheckYourAnswersPage = await checkYouranswers.isPresent();
    if (isCheckYourAnswersPage) {
      const submit = element(by.xpath('//button[@type= "submit"]'));
  ...
  async startNextStep(stepName, isAccessibilityTest){
    ...
    const nextStepSelect = element(by.xpath('//*[@id=\'next-step\']'));
  ...
  async _formFillPage(pageCounter) {
    ...
    const fields = element.all(by.css(this.formFields));
    for (let count = 0; count < await fields.count(); count++) {
      const isHiddenElement = await fields.get(count);
      ...
      const fieldElement = await fields.get(count);
      const field = await fieldElement.element(by.xpath('./*'));
      const readWriteField = await field.getTagName();

      if (readWriteField.toLowerCase() === 'ccd-field-write') {
        const ccdField = await field.element(by.xpath('./div//*'));
    ...
    const nextPageUrl = await browser.getCurrentUrl();
  ...
  async _writeToField(ccdField, parentFieldName) {
    ...
    let ccdFileTagName = await ccdField.getTagName();
    ...
    switch (ccdFileTagName) {
      case 'ccd-write-text-field':
        const e = await ccdField.$('input.form-control');
      ...
      case 'ccd-write-text-area-field':
        const e1 = await ccdField.$('textarea.form-control');
      ...
      case 'ccd-write-money-gbp-field':
        const e2 = await ccdField.$('input.form-control');
        const gbpvalue = 300;
      ...
      case 'ccd-write-number-field':
        const e3 = await ccdField.$('input.form-control');
      ...
      case 'ccd-write-phone-uk-field':
        const e4 = await ccdField.$('input.form-control');
      ...
      case 'ccd-write-address-field':
        await BrowserWaits.retryWithActionCallback(async () => {
          ...
          const addressSelectionField = ccdField.$('select.form-control');

          // await addressSelectionField.clickSelectOption(2)

          await BrowserWaits.waitForElement(addressSelectionField);
          const addressToSelectOption = addressSelectionField.$('option:nth-of-type(2)');
          await BrowserWaits.retryWithActionCallback(async () => {
            const addressOptions = await addressSelectionField.getSelectOptions();
      ...
      case 'ccd-write-yes-no-field':
        ...
        const yesOrNoFieldElement = ccdField.$$('.multiple-choice label');
        const selectionYesorNoValue = await yesOrNoFieldElement.get(0).getText();
      ...
      case 'ccd-write-fixed-list-field':
        ...
        // }
        const selectFieldId = await ccdField.$('select');
      ...
      case 'ccd-write-document-field':
        await BrowserWaits.retryWithActionCallback(async () => {
          ...
          await BrowserWaits.waitForCondition(async () => {
            const isStatusDisplayed = await statusMessageELement.isDisplayed();
      ...
      case 'ccd-write-fixed-radio-list-field':
        ...
        const radioElement = await selectionRadioFields.get(0);
      ...
      case 'ccd-write-complex-type-field':
        ...
        for (let fieldcounter = 0; fieldcounter < await writeFields.count(); fieldcounter++){
          const isHidden = await (await writeFields.get(fieldcounter)).getAttribute('hidden');
          if (isHidden){
            continue;
          }
      ...
      case 'ccd-write-collection-field':
        ...
        for (let i = 0; i < 3; i++){
          ...
          for (var count = 0; count < writeFieldsCount; count++) {
            ...
            const subFieldText = await ccdSubField.getText();
