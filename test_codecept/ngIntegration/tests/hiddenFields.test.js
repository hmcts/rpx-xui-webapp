...
describe('CCD casefields, retain_hidden_field setting', function () {
  ...
  [
    ...
    it(`Text Field : retain_hidden_field "${scenario.retainHiddenField}" and field is Displayed ${scenario.showField}`, async function () {
      ...
      const testFieldShowYesNo = caseConfig.addCCDFieldToPage(page1, 'YesOrNo', 'showTestField', 'Show Test Field?');
      testFieldShowYesNo.value = true;

      const textField = caseConfig.addCCDFieldToPage(page1, 'Text', 'simpleTextField', 'Test old value');
  ...
  [
    ...
    it(`field ${scenario.fieldType} : retain_hidden_field "${scenario.retainHiddenField}" and field is Displayed ${scenario.showField}`, async function () {
      ...
      switch (scenario.fieldType) {
        ...
        case 'Document':
          complexField.value = {
            ...
            document_binary_url:
              'http://dm-store-aat.service.core-compute-aat.internal/documents/a612199d-9972-4b99-b653-5ec7c310e21a/binary',
      ...
      if (!scenario.showField && scenario.retainHiddenField === 'No') {
        ...
        expect(validateAllValuesNullInObject(caseEventSubmitRequestBody.data[complexField.id]), 'object has non null values').to
          .be.true;
      }
  ...
  [
    ...
    it(`field ${scenario.fieldType} : retain_hidden_field "${scenario.retainHiddenField}" and field is Displayed ${scenario.showField}`, async function () {
      ...
      let complexField = caseConfig.addCCDFieldToPage(
        page1,
        scenario.fieldType,
        scenario.fieldType + 'Field',
        scenario.fieldType + ' Field'
      );
      ...
      switch (scenario.fieldType) {
        case 'Collection':
          ...
          const complexCollectionField = caseConfig.getCCDFieldTemplateCopy(
            'Complex',
            'ComplexCollectionItems',
            'Collection Item'
          );
  ...
  [
    ...
    it(`Deep complex field : ${scenario.testName}`, async function () {
      ...
      const parentComplexShowYesNo = caseConfig.addCCDFieldToPage(
        page1,
        'YesOrNo',
        'parentComplexShowYesNo',
        'Show Parent Complex?'
      );
      ...
      const childComplexShowCondition = caseConfig.getCCDFieldTemplateCopy(
        'YesOrNo',
        'childComplexShowCondition',
        'Show Child Complex?'
      );
      ...
      const grandchildTextShowCindition = caseConfig.getCCDFieldTemplateCopy(
        'YesOrNo',
        'grandchildTextShowCindition',
        'Show Grandchild Text Field?'
      );
      ...
      await CCDCaseEditPage.inputCaseField(
        parentComplexField.id + '_' + childComplex.id + '_' + grandchildText.id,
        'Grand Child text value new'
      );
      ...
      if (scenario.parentComplex.display) {
        await CCDCaseEditPage.selectRadioYesOrNo(
          parentComplexField.id + '_' + childTextShowCondition.id,
          scenario.childText.display
        );
        await CCDCaseEditPage.selectRadioYesOrNo(
          parentComplexField.id + '_' + childComplexShowCondition.id,
          scenario.childComplex.display
        );

        if (scenario.childComplex.display) {
          await CCDCaseEditPage.selectRadioYesOrNo(
            parentComplexField.id + '_' + childComplex.id + '_' + grandchildTextShowCindition.id,
            scenario.grandchildText.display
          );
        }
      }
      ...
      if (scenario.parentComplex.display && scenario.childText.display) {
        expect(caseEventSubmitRequestBody.data[parentComplexField.id][childText.id], 'For field' + childText.id).to.equal(
          'Child text value new'
        );
      } else if (scenario.childText.retainValue) {
        expect(caseEventSubmitRequestBody.data[parentComplexField.id][childText.id], 'For field' + childText.id).to.equal(
          'Child text value old'
        );
      } else {
      ...
      if (scenario.parentComplex.display && scenario.childComplex.display && scenario.grandchildText.display) {
        expect(
          caseEventSubmitRequestBody.data[parentComplexField.id][childComplex.id][grandchildText.id],
          'For field' + grandchildText.id
        ).to.equal('Grand Child text value new');
      } else if (scenario.childText.retainValue) {
        expect(
          caseEventSubmitRequestBody.data[parentComplexField.id][childComplex.id][grandchildText.id],
          'For field' + grandchildText.id
        ).to.equal('Grand Child text value old');
      } else {
        expect(
          caseEventSubmitRequestBody.data[parentComplexField.id][childComplex.id][grandchildText.id],
          'For field' + grandchildText.id
  ...
  function setUpcaseConfig(caseConfig) {
    ...
    MockApp.onPost('/data/case-types/:caseType/validate', (req, res) => {
      ...
      const responseBody = {
        ...
        _links: {
          self: {
            href: 'http://ccd-data-store-api-aat.service.core-compute-demo.internal' + req.path + '?pageId=' + req.query.pageId,
          },
        },
      };
    ...
    MockApp.onPost('/data/cases/:caseid/events', (req, res) => {
      ...
      const responseBody = {
        ...
        _links: {
          self: {
            href: 'http://ccd-data-store-api-demo.service.core-compute-demo.internal' + req.path + '?ignore-warning=false',
          },
        },
      };
      res.send(responseBody);
