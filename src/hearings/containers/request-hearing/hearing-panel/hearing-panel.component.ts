...
export class HearingPanelComponent extends RequestHearingPageFlow implements OnInit, AfterViewInit, OnDestroy {
  ...
  public childNodesValidation(): boolean {
    ...
    panelRoles
      .filter((panelRole) => panelRole.selected && panelRole.child_nodes?.length)
  ...
  public getPanelMemberList(panelRequirementType: string): JudicialUserModel[] {
    ...
    const panelMemberIDs = panelRequirements?.panelPreferences
      ?.filter(
        (preferences) =>
          preferences.memberType === MemberType.PANEL_MEMBER && preferences.requirementType === panelRequirementType
      )
    ...
    if (multi.child_nodes && multi.child_nodes.length) {
      multi.child_nodes.forEach((node) => {
        if (node.key.toLowerCase().trim() === panelSpecialism.toLocaleLowerCase().trim() && !skip && !multi.selected) {
          ...
          skip = node.child_nodes && node.child_nodes.length ? this.loadPanel(node, panelSpecialism) : true;
        }
      });
    } else {
  ...
  public loadHearingPanels(): void {
    ...
    this.panelSelection = '';
    if (this.multiLevelSelections?.length && this.hearingRequestMainModel.hearingDetails?.panelRequirements?.roleType) {
      selectedPanelRoles = this.hearingRequestMainModel.hearingDetails?.panelRequirements.roleType.filter((roleKey) =>
        this.multiLevelSelections.map((role) => role.key).includes(roleKey)
      );
      selectedPanelRoles.forEach((selectedPanelRole) => {
        ...
        if (this.multiLevelSelections?.length) {
          this.multiLevelSelections.forEach((role) => {
            if (
              role.key?.toLowerCase().trim() === selectedPanelRole?.toLocaleLowerCase().trim() &&
              !skipRoleSelection &&
              !role.selected
            ) {
              role.selected = true;
  ...
  public prepareData(): void {
    const panelRoles: LovRefDataModel[] = this.convertArrayToRefDataModel(
      this.panelJudgeForm.controls.multiLevelSelect as FormArray
    );
    ...
    if (hearingPanelRequiredFlag) {
      ...
      selectedPanelRoles =
        (panelRoles && panelRoles.filter((role) => role.selected).map((role) => role.selected && role.key)) || [];
    }
    const panelRequirements = this.hearingRequestMainModel.hearingDetails.panelRequirements;
    const preSelectedPanelRoles = panelRequirements?.roleType?.filter(
      (roleKey) => !panelRoles.map((role) => role.key).includes(roleKey)
    );
    const selectedPanelJudges: PanelPreferenceModel[] =
      panelRequirements?.panelPreferences?.filter((preferences) => preferences.memberType === MemberType.JUDGE) || [];
  ...
  public isFormValid(): boolean {
    ...
    if (panelRequiredFlag) {
      const selectedPanelRoles: LovRefDataModel[] = this.convertArrayToRefDataModel(
        this.panelJudgeForm.controls.multiLevelSelect as FormArray
