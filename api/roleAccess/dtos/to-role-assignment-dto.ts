...
export function toSARoleAssignmentBody(
  ...
  specificAccessData: { specificAccessStateData: AllocateRoleData; period: Period },
  ...
  const requestedRole =
    allocateRoleData.requestedRole !== 'specific-access-judicial' ? allocateRoleData.requestedRole : 'specific-access-judiciary';
  return {
    roleRequest: {
      assignerId: currentUserId,
...
export function toSARequestRoleAssignmentBody(
  allocateRoleData: AllocateRoleData,
  extraAttributesForBasicRole: { [x: string]: string | boolean } = {}
) {
  const todayDate = new Date();
    ...
    requestedRoles: [
      {
        ...
        beginTime: new Date(),
        endTime:
          allocateRoleData.period?.endDate ? allocateRoleData.period.endDate
