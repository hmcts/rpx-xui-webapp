...
Given(
  'I note task id from table at position {int} with reference {string} to track',
  async function (ttaskAtpos, taskIdReference) {
...
Then('I see task action links', async function (actionsTable) {
  ...
  for (const actionRow of actionRows) {
    ...
    expect(
      await taskListPage.isTaskActionPresent(action),
      `Task action "${action} ${JSON.stringify(actionRow, 2)}" is not displayed`
    ).to.be.true;
  }
});
...
Then('I see task action suceess confirmation banner', async function () {
  ...
  const isSuccessDidplayed = await taskListPage.isBannermessageWithTextDisplayed(
...
When('I claim available task and note taskId wuth reference {string}', async function (taskIdReference) {
  ...
  for (let i = 1; i <= tasksCount; i++) {
    ...
    expect(await taskListPage.isTaskActionBarDisplayedForAtPos(i), 'Task actions row not displayed for task at row ' + i).to.be
      .true;
    await taskListPage.clickTaskAction('Assign to me');
    expect(await taskListPage.isBannerMessageDisplayed(), 'Message banner not displayed').to.be.true;
    const messages = await taskListPage.getBannerMessagesDisplayed();
    const isMessageDisplayed = await taskListPage.isBannermessageWithTextDisplayed(
      "You've assigned yourself a task. It's available in My tasks"
    );
...
Then(
  'I validate task one click action landing on {string} page and message displayed {string}',
  async function (landingPage, messageDisplayed) {
  ...
  for (let i = 0; i < releaseColumnHashes.length; i++) {
    ...
    if (columnName && columnName !== '') {
      ...
      await softAssert.assert(
        async () => expect(isLink, `${columnName} with text ${await firstRowColumnValElement.getText()} is not a link`).to.be.true
      );
...
Then('I validate tasks pagination is displayed if feature toggle {string} is on', async function (featureToggleName) {
  ...
  expect(await taskListPage.paginationContainer.isDisplayed(), 'Pagination display is expected to be ' + toggleVal).to.equal(
    toggleVal
  );
});
