...
test.describe('Document upload helper unit tests', { tag: '@svc-internal' }, () => {
  test('uploadDocumentViaApi posts through browser fetch on the current app origin with the XSRF header from browser-context cookies', async () => {
    ...
    const page = {
      ...
      url: () => 'https://xui-webapp-pr-4989.preview.platform.hmcts.net/cases',
      evaluate: async (
        _fn: unknown,
        request: {
          url: string;
    ...
    expect(captured.url).toBe('https://xui-webapp-pr-4989.preview.platform.hmcts.net/documentsv2');
  ...
  test('uploadDocumentViaApi fails fast when the XSRF cookie never appears', async () => {
    ...
    try {
      await expect(
        ...
        /Document upload setup failed: XSRF-TOKEN cookie was not available within 20000ms \(url=https:\/\/manage-case\.aat\.platform\.hmcts\.net\/cases, cookies=none\)/
      );
    } finally {
  ...
  test('uploadDocumentViaApi refreshes the current app shell before giving up on XSRF minting', async () => {
    ...
    expect(navigations).toEqual(['https://manage-case.aat.platform.hmcts.net/cases']);
    expect(requestGets.some((url) => url.endsWith('/auth/login'))).toBe(true);
