...
test.describe('IDAM login using credentials for Global Search', () => {
  ...
  test("Global Search (Partial) - using '*' wildcard on caseNumeber and party name ", async ({
    globalSearchPage,
    page,
    tableUtils,
  }) => {
    ...
    await test.step('Verify wildcard (*) search results ', async () => {
      ...
      expect(globalSearchPage.changeSearchLink.filter({ has: page.getByRole('link', { name: ' Change search ' }) }).isVisible());
