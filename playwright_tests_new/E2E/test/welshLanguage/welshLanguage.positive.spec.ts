...
test.describe('Verify users can switch the language', () => {
  ...
  test('Verify translations are shown when the user selects to view the site in Welsh', async ({ caseListPage, waitUtils }) => {
    await test.step('Change the language to Welsh', async () => {
      ...
      const cyResponse = await caseListPage.page
        .waitForResponse((res) => res.url().includes('/api/translation/cy') && res.ok(), { timeout: 15000 })
        .catch(() => null);
      if (!cyResponse) {
    ...
    await test.step('Check the language can be switched back to English and the correct translations are shown', async () => {
      ...
      const enResponsePromise = caseListPage.page
        .waitForResponse((res) => res.url().includes('/api/translation/en') && res.ok(), { timeout: 10000 })
        .catch(() => null);
      const englishRenderedPromise = caseListPage.page
        .waitForFunction(
          () => {
