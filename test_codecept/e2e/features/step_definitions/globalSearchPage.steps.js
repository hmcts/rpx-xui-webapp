...
//noResultsFoundPageSearchAgainLink
When('I click Search again link in global search results page', async function(){
  await globalSearchResultsPage.noResultsFoundPageSearchAgainLink.click();
});
...
Then('I validate global search results - No results found displayed', async function () {
  await BrowserWaits.retryWithActionCallback(async () => {
    expect((await globalSearchResultsPage.getTableRowsCount()) > 0, 'No results found').to.be.true;
  });
});
...
Then('I validate global search results pagination link {string} is enabled', async function(paginationLink){
  if (paginationLink.toLowerCase().includes('next')){
   ...
   // expect(await globalSearchResultsPage.nextpageLink.getTagName()).to.not.be.null();


  }else{
    //expect(await globalSearchResultsPage.previousPageLink.getTagName()).to.equal('a')
   // expect(await globalSearchResultsPage.previousPageLink.getTagName()).to.not.be.null()
  }
