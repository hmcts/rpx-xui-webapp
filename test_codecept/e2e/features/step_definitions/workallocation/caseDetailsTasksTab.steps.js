...
When(
  'I click manage link {string} for task at position {int} in case details tasks tab',
  async function (manageLinkText, taskPos) {
...
When(
  'I click active tast attribute Next steps link {string} for task at position {int} with name {string}',
  async function (attributeLinktext, position, taskNameExpected) {
...
Then(
  'I validate task tab active task at position {int} with task name {string} has attributes',
  async function (position, taskNameExpected, attributesDatatable) {
    ...
    await BrowserWaits.retryWithActionCallback(async () => {
      ...
      for (const attributeHash of expectedAttributeHashes) {
        ...
        reportLogger.AddMessage(
          `Validating task attributes ${attribName}, attributeIsDisplayed ${attributeIsDisplayed}, validateContentType ${validateContentType}, content ${contentText}`
        );
        ...
        if (attributeIsDisplayed.includes('true') || attributeIsDisplayed.includes('yes')) {
          ...
          await softAssert.assert(async () =>
            expect(Object.keys(taskAttributes), `Task details missing : ${attribName}`).to.includes(attribName)
          );
          ...
          if (validateContentType.toLowerCase().includes('link')) {
              ...
              if (isLinkWithTextPresent) {
                if (validateContentType.toLowerCase().includes('urlcontains')) {
                  ...
                  softAssert.assert(async () => {
                    expect(linkHref.includes(expectedHref), `link href ${linkHref} not not contains "${expectedHref}"`).to.be
                      .true;
                  });
                }
                break;
            ...
            await softAssert.assert(
              async () => expect(isLinkWithTextPresent).to.be.true,
              `${linktext} to include ${contentText}`
            );
          }
        } else {
