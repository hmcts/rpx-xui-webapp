...
class TestDataManager {
  async getS2SToken() {
    const res = await axios.post(
      `http://rpe-service-auth-provider-${appTestConfig.testEnv}.service.core-compute-${appTestConfig.testEnv}.internal/testing-support/lease`,
      {
  ...
  async cleanupAutoStaffUsers() {
    try {
      ...
      await axios.delete(
        `https://idam-api.${appTestConfig.testEnv}.platform.hmcts.net/testing-support/test-data?testDataPrefix=xui_auto&async=true`
      );

      await axios.delete(
        `http://rd-caseworker-ref-api-${appTestConfig.testEnv}.service.core-compute-${appTestConfig.testEnv}.internal/refdata/case-worker/users?emailPattern=xui_auto`,
        {
          headers: {
