...
export class SpecificAccessEffects {
  public approveSpecificAccessRequest$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((data) =>
        this.allocateRoleService.specificAccessApproval(data.payload.specificAccessStateData, data.payload.period).pipe(
          map(() => {
  ...
  public requestMoreInfoSpecificAccessRequest$ = createEffect(() =>
    this.actions$.pipe(
      ofType<RequestMoreInfoSpecificAccessRequest>(SpecificAccessActionTypes.REQUEST_MORE_INFO_SPECIFIC_ACCESS_REQUEST),
      mergeMap((data) =>
        this.allocateRoleService.requestMoreInformation(data.payload).pipe(
          map(() => {
