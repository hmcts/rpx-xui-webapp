...
class WorkAllocationMockData {
  ...
  setDefaultData() {
    ...
    this.usersFromServices = this.getUsersFromServices(this.waSupportedJurisdictions);
      ...
      for (const taskAttribute of taskAttributes) {
        if (taskAttribute.toLowerCase().includes('date')) {
          ...
          if (val.includes('session')) {
            ...
            taskTemplate[taskAttribute] = nodeAppMock.userDetails.userInfo.uid
              ? nodeAppMock.userDetails.userInfo.uid
              : nodeAppMock.userDetails.userInfo.id;
