...
describe('LocationResolver', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [StoreModule.forRoot(reducers, { metaReducers }), RouterTestingModule.withRoutes([])],
      providers: [
        LocationResolver,
        AllocateRoleService,
        LocationDataService,
    ...
    const expectedLocationList = [
      {
        ...
        locations: [
          {
            id: '12345',
            userId: '998db99b-08aa-43d4-bc6b-0aabbb0e3c6f',
            locationId: '12345',
            locationName: '',
            services: ['IA'],
          },
  ...
  it('should get locations for user within region', inject([LocationResolver], (service: LocationResolver) => {
    ...
    const locationsByRegion: LocationsByRegion[] = [
      { regionId: '4', locations: ['12345'] },
      { regionId: '3', locations: ['512345'] },
    ];
    const expectedLocationList = [
      {
        service: 'IA',
        locations: [{ userId: '998db99b-08aa-43d4-bc6b-0aabbb0e3c6f', locationName: '', services: ['IA'], regionId: '3' }],
      },
    ];
  ...
  it('should get base locations for user within region', inject([LocationResolver], (service: LocationResolver) => {
    ...
    // EUI-7909 - remove next four lines
    const expectedLocation = [
      { id: '54321', userId: '998db99b-08aa-43d4-bc6b-0aabbb0e3c6f', locationName: '', locationId: '54321', services: ['IA'] },
    ];
    const locationsByRegion: LocationsByRegion[] = [
      { regionId: '4', locations: ['12345'] },
      { regionId: '3', locations: ['54321'] },
    ];
    const expectedLocationList = [
      {
        service: 'IA',
  ...
  it('should get base locations for user not in region', inject([LocationResolver], (service: LocationResolver) => {
    ...
    // EUI-7909 - remove next four lines
    const expectedLocation = [
      { id: null, userId: '998db99b-08aa-43d4-bc6b-0aabbb0e3c6f', locationName: '', locationId: null, services: ['IA'] },
    ];
    const locationsByRegion: LocationsByRegion[] = [
      { regionId: '4', locations: ['12345', '2341'] },
      { regionId: '3', locations: ['54321'] },
    ];
    const expectedLocationList = [{ service: 'IA', locations: [] }];
    expect(service.getJudicialWorkersOrCaseWorkers(locationsByRegion, CASE_WORKER_WITH_BASE_NOT_IN_REGION)).toEqual(
      expectedLocation
    );
    expect(sessionStorageService.setItem).toHaveBeenCalledWith('userLocations', JSON.stringify(expectedLocationList));
  }));

  it('should get all relevant locations for user without location restrictions on role assignment', inject(
    [LocationResolver],
  ...
  it('should get all relevant locations for user without location restrictions on individual role assignment', inject(
    [LocationResolver],
    (service: LocationResolver) => {
      spyOn(sessionStorageService, 'setItem');
      // EUI-7909 - uncomment out code
 ...
 */
      ...
      expect(service.getJudicialWorkersOrCaseWorkers(locationsByRegion, CASE_WORKER_SHOULD_HAVE_ALL_LOCATIONS)).toEqual(
        expectedLocation
      );
      expect(sessionStorageService.setItem).toHaveBeenCalledWith('userLocations', '[]');
    }
  ...
  it('should get all relevant locations for fee paid judge user for booking UI', inject(
    [LocationResolver],
    (service: LocationResolver) => {
      spyOn(sessionStorageService, 'setItem');
      const locationsByRegion: LocationsByRegion[] = [
