...
export class RemoveRoleComponent implements OnInit {
    ...
    paramMap$
      ...
      .subscribe((caseRoles: CaseRole[]) => {
        ...
        if (!this.role.email && this.role.actorId) {
          const caseworkers = safeJsonParseFallback<Caseworker[]>(this.sessionStorageService.getItem('caseworkers'), []);
          if (caseworkers) {
  ...
  public getRoleAssignmentFromQuery(queryMap: ParamMap): Observable<CaseRole[]> {
    ...
    return this.allocateRoleService
      .getCaseRoles(this.caseId, this.jurisdiction, caseType, this.assignmentId)
      .pipe(
        mergeMap((caseRoles: CaseRole[]) =>
          this.allocateRoleService
            .getCaseRolesUserDetails(Utils.getJudicialUserIds(caseRoles), [this.jurisdiction])
            .pipe(map((caseRolesWithUserDetails: CaseRoleDetails[]) => Utils.mapCaseRoles(caseRoles, caseRolesWithUserDetails)))
        )
      );
  }
