...
export class CaseFlagsUtils {
  ...
  public static getReasonableAdjustmentFlags(
    ...
    const reasonableAdjustmentFlags = activeFlags?.filter(
      (caseFlag) =>
  ...
  public static getNonReasonableAdjustmentFlagsGroupedByPartyName(
    ...
    partiesInHMC: PartyDetailsModel[],
    partiesInSHV: PartyDetailsModel[],
    hearingDetails: RequestDetailsModel,
    ...
    return this.getNonReasonableAdjustmentFlagsGroup(
      groupedFlags,
      partiesInHMC,
      partiesInSHV,
      hearingDetails,
  ...
  public static getNonReasonableAdjustmentFlags(
    caseFlagsRefData: CaseFlagReferenceModel[],
    caseFlags: PartyFlagsModel[],
    partiesInSHV: PartyDetailsModel[]
    ...
    const nonReasonableAdjustmentPartyFlags =
      activeFlags?.filter((activeFlag) => !reasonableAdjustmentFlags.includes(activeFlag)) || [];
   ...
   */
  public static displayCaseFlagsGroup(
    ...
    const allRAFs = allActiveFlags.filter(
      (caseFlag) =>
        caseFlag.displayPath.includes(CaseFlagType.REASONABLE_ADJUSTMENT) ||
        caseFlag.flagId === CaseFlagsUtils.LANGUAGE_INTERPRETER_FLAG_ID
    );
  ...
  private static getAllActiveDisplayFlags(
    partyFlags: PartyFlagsModel[],
    caseFlagsRefDataModels: CaseFlagReferenceModel[]
  ): PartyFlagsDisplayModel[] {
  ...
  public static convertPartiesToPartyWithFlags(
    caseFlagReferenceModels: CaseFlagReferenceModel[],
    partyDetails: PartyDetailsModel[],
    partiesFromServiceValue?: PartyDetailsModel[]
    ...
    partyDetails.forEach((party) => {
      ...
      const allFlagIds: string[] = reasonableAdjustments.slice();
      const flags = this.addLanguageFlagIfMissing(allFlagIds, party?.individualDetails);

      const allFlags: CaseFlagReferenceModel[] = flags
        .map((flagId) => CaseFlagsUtils.findFlagByFlagId(caseFlagReferenceModels, flagId))
  ...
  public static convertPartiesToPartyWithReasonableAdjustmentFlags(
    ...
    individualParties?.forEach((party) => {
      ...
      if (partyName) {
        const reasonableAdjustments =
          party.individualDetails?.reasonableAdjustments?.filter((reasonableAdjustment) =>
            reasonableAdjustment.startsWith('RA')
          ) || [];
        const flagIds = reasonableAdjustments.slice();
        const flags = this.addLanguageFlagIfMissing(flagIds, party?.individualDetails);

        const allFlags: CaseFlagReferenceModel[] = flags.map((flagId) =>
          CaseFlagsUtils.findFlagByFlagId(caseFlagReferenceModels, flagId)
        );
        partyWithFlags.set(partyName, allFlags);
      }
    });
  ...
  public static addLanguageFlagIfMissing(flagIds: string[], individualDetails: IndividualDetailsModel): string[] {
    if (individualDetails?.interpreterLanguage) {
      if (!flagIds.includes(this.SIGN_LANGUAGE_INTERPRETER_FLAG_ID)) {
        flagIds.push(this.LANGUAGE_INTERPRETER_FLAG_ID);
      }
    }
    return flagIds;
  }
  ...
  private static getActiveDisplaysFlags(
    caseFlags: PartyFlagsModel[],
    caseFlagsRefData: CaseFlagReferenceModel[],
    partiesInSHV: PartyDetailsModel[]
  ): PartyFlagsDisplayModel[] {
  ...
  private static getReasonableAdjustmentFlagsGroup(
    groupedReasonableAdjustmentFlags: Record<string, PartyFlagsDisplayModel[]>,
    partiesInHMC: PartyDetailsModel[],
    partiesInSHV: PartyDetailsModel[]
  ): CaseFlagGroup[] {
    ...
    for (const reasonableAdjustmentFlag in groupedReasonableAdjustmentFlags) {
      if (groupedReasonableAdjustmentFlags.hasOwnProperty(reasonableAdjustmentFlag)) {
        reasonableAdjustmentFlagGroups.push({
          ...
          partyFlags: this.getReasonableAdjustmentFlagsWithAmendedLabelStatus(
            groupedReasonableAdjustmentFlags[reasonableAdjustmentFlag],
            partiesInHMC,
            partiesInSHV
          ),
          partyAmendmentLabelStatus: this.getPartyAmendmentLabelStatus(
            groupedReasonableAdjustmentFlags[reasonableAdjustmentFlag],
            partiesInHMC,
            partiesInSHV
          ),
    ...
    for (const flag in groupedFlags) {
      if (groupedFlags.hasOwnProperty(flag)) {
        nonReasonableAdjustmentFlagGroups.push({
          name: flag,
          partyFlags: this.getNonReasonableAdjustmentFlagsWithAmendedLabelStatus(
            groupedFlags[flag],
            hearingDetails,
            nonReasonableAdjustmentChangesConfirmed
          ),
          partyAmendmentLabelStatus: nonReasonableAdjustmentChangesConfirmed
            ? AmendmentLabelStatus.NONE
            : this.getPartyAmendmentLabelStatus(groupedFlags[flag], partiesInHMC, partiesInSHV),
        } as CaseFlagGroup);
      }
    }
  ...
  private static getNonReasonableAdjustmentFlagsWithAmendedLabelStatus(
    ...
    if (!nonReasonableAdjustmentChangesConfirmed) {
      for (const flag of flags) {
        if (
          (flag.dateTimeModified && new Date(flag.dateTimeModified) > new Date(hearingDetails.timestamp)) ||
  ...
  private static getPartyAmendmentLabelStatus(
    flags: PartyFlagsDisplayModel[],
    partiesInHMC: PartyDetailsModel[],
    partiesInSHV: PartyDetailsModel[]
