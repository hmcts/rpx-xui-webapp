...
class HearingCreateEditSummaryPage {
  ...
  async validateSummaryFieldWithValueDisplayed(section, field, value) {
    ...
    if (section.trim() !== '') {
      ...
      expect(
        await elementByXpath(fieldLevelLocators.changeLinkElement).first().isVisible(),
        `hearing field ${field} change link not displayed`
      ).to.be.true;
    } else {
      expect(
        await elementByXpath(fieldLevelLocators.changeLinkElement).first().isVisible(),
        `case field ${field} change link displayed`
      ).to.be.false;
    }
  }
}
