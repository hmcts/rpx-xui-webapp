...
class HearingWelshPage {
  ...
  get fieldMapping() {
    return {
      'Does this hearing need to be in Welsh?': elementByXpath(
        "//exui-hearing-welsh//h1[contains(text(),'Does this hearing need to be in Welsh?')]"
      ),
    };
  }
    ...
    switch (field) {
      case 'Does this hearing need to be in Welsh?':
        ...
        const ele = elementByXpath(
          `//exui-hearing-welsh/div/form/div/fieldset/div[2]/${div}/label[contains(text(),'${value}')]/../input`
        );
        await ele.click();
        break;
      default:
