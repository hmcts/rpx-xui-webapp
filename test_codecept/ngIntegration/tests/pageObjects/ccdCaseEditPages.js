...
class CaseEdit {
  ...
  async inputYesOrNoField(fieldConfig, inputOption, parentId) {
    ...
    if (parentId) {
      inputoptionId = parentId +'_'+ inputoptionId;
    }
  ...
  async inputaddressGlobalUK(fieldConfig, value, parentid) {
    ...
    if (parentid == undefined) {
      complexId = `${fieldConfig.id}_${fieldConfig.id}`;
    }
    if (parentid) {
      complexId = `${parentid}_${fieldConfig.id}_${fieldConfig.id}`;
    }
    ...
    for (const complexFiedlConfig of fieldConfig.field_type.complex_fields) {
      ...
      const value = await $(`#${prefix}${complexFiedlConfig.id}`).getAttribute('value');
  ...
  async inputDate(fieldConfig, dateVal, parentId) {
    ...
    const datesValues = inputDate.split('-');
