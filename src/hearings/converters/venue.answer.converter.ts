  ...
  public transformAnswer(hearingState$: Observable<State>): Observable<string> {
    return hearingState$.pipe(
      ...
      switchMap((state) => {
        ...
        const locationIds = hearingLocations
          .map((hearingLocationModel: HearingLocationModel) => hearingLocationModel.locationId)
          .join(',');
        return this.locationsDataService.getLocationById(locationIds, serviceCode).pipe(
          map((LocationByEpimmsModels: LocationByEpimmsModel[]) => {
            let result = '<ul>';
            LocationByEpimmsModels.forEach(
              (LocationByEpimmsModel: LocationByEpimmsModel) => (result += `<li>${LocationByEpimmsModel.court_name}</li>`)
            );
            result += '</ul>';
            return result;
          })
