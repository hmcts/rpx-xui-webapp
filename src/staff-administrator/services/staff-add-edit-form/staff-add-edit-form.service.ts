...
export class StaffAddEditFormService {
  ...
  public get valuesAsStaffUser() {
    ...
    staffUser.region = this.staffFilterOptions.regions.find(
      (item) => Number(item.region_id) === staffUser.region_id
    )?.description;
    staffUser.roles = filterItemsByBoolean<StaffFilterOption>(this.staffFilterOptions.jobTitles, formValues.roles).map(
      (item) => ({ role_id: item.key, role: item.label, is_primary: true })
    );
    staffUser.services = filterItemsByBoolean<StaffFilterOption>(this.staffFilterOptions.services, formValues.services).map(
      (item) => ({ service: item.label, service_code: item.key })
    );
  ...
  public patchFormValues(staffUserDetails: StaffUser) {
    ...
    // Services
    const serviceFormArray = buildCheckboxArray(
    ...
    // Job titles
    const jobTitlesFormArray = buildCheckboxArray(
    ...
    this.staffFilterOptions.skills.forEach((item) => {
      ...
      item.options.forEach((option) => {
        const checked = staffUserDetails.skills
          ? staffUserDetails.skills.some((skill) => skill.skill_id === Number(option.key))
          : false;
