    ...
    return hearingState$.pipe(
      map((state) => {
        ...
        const objA =
          ...
          objAPanelRequirements.panelPreferences.filter(
            (panel) => panel.memberType === MemberType.JUDGE && panel.requirementType === RequirementType.EXCLUDE
          );
        const objB =
          objBPanelRequirements &&
          objBPanelRequirements.panelPreferences.filter(
            (panel) => panel.memberType === MemberType.JUDGE && panel.requirementType === RequirementType.EXCLUDE
          );
