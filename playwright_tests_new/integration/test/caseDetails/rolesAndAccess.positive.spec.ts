...
test.describe(`Roles and access as ${userIdentifier}`, { tag: ['@integration', '@integration-case-details'] }, () => {
  ...
  test('Case allocator sees active roles, exclusions, headers, and manage actions', async ({
    ...
    await test.step('Verify EXCLUDED grant behaviour and allocator controls are rendered', async () => {
      ...
      const judicialReallocateUrl = new URL((await judicialReallocateLink.getAttribute('href')) ?? '', 'https://xui.local');
      const judicialRemoveUrl = new URL((await judicialRemoveLink.getAttribute('href')) ?? '', 'https://xui.local');
      ...
      const legalOpsReallocateUrl = new URL((await legalOpsReallocateLink.getAttribute('href')) ?? '', 'https://xui.local');
