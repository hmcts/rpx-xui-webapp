...
When(
  'I click work allocation cases pagination link {string} and wait for req reference {string} not null',
...
Then(
  'I validate work allocation cases search request with reference {string} has pagination parameters',
...
Then(
  'I validate work allocation cases search request with reference {string} have search parameters',
  async function (requestReference, datatable) {
    ...
    for (let i = 0; i < datatableHash.length; i++) {
      ...
      if (searchHash.keyIsPresent) {
        await softAssert.assert(async () =>
          expect(searchParamObj.length > 0).to.equal(searchHash.keyIsPresent.toLowerCase() === 'true')
        );
      } else {
...
Then(
  'I validate work allocation cases search request with reference {string} does not have search parameters',
  async function (requestReference, datatable) {
    const softAssert = new SoftAssert();
    const reqBody = global.scenarioData[requestReference];
    const datatableHash = datatable.parse().hashes();
