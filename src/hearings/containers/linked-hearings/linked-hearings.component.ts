...
export class LinkedHearingsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.sub = this.hearingStore.pipe(select(fromHearingStore.getHearingValuesModel)).subscribe((state) => {
        ...
        tap((caseInfo) => {
          this.hearingStore.dispatch(
            new fromHearingStore.LoadServiceLinkedCases({
              jurisdictionId: caseInfo.jurisdictionId,
              caseReference: caseInfo.caseReference,
              hearingId: this.hearingId,
            })
          );
          this.hearingStore.dispatch(
            new fromHearingStore.LoadServiceLinkedCasesWithHearings({
              jurisdictionId: caseInfo.jurisdictionId,
              caseReference: caseInfo.caseReference,
