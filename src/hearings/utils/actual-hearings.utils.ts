...
export class ActualHearingsUtils {
  ...
  public static getActualHearingDays(
    hearingActualsMainModel: HearingActualsMainModel,
    isCheckYourAnswersPage: boolean
  ): ActualHearingDayModel[] {
    ...
    hearingDays = hearingActualsMainModel.hearingPlanned.plannedHearingDays.map((plannedDay) => {
      ...
      return {
        hearingDate: existingActualData?.hearingDate || this.getDate(plannedDay.plannedStartTime),
        hearingStartTime: existingActualData?.hearingStartTime
          ? existingActualData.hearingStartTime
    ...
    if (hearingDays && hearingDays.length > 0) {
      hearingDays = hearingDays.sort((a, b) => {
        return Date.parse(a.hearingDate) === Date.parse(b.hearingDate)
          ? 0
          : Date.parse(a.hearingDate) > Date.parse(b.hearingDate)
            ? 1
            : -1;
      });
    }
  ...
  public static getActualDayParties(
    existingActualData: ActualHearingDayModel,
    plannedDay: PlannedHearingDayModel,
    isCheckYourAnswersPage: boolean
  ): ActualDayPartyModel[] {
    if (isCheckYourAnswersPage) {
  ...
  public static mergeSingleHearingPartActuals(
    hearingActualsMainModel: HearingActualsMainModel,
    hearingDate: string,
    updatedActuals: ActualHearingDayModel
  ): HearingActualsModel {
    ...
    if (hearingActuals.actualHearingDays?.length > 0) {
      indexOfActual = hearingActuals.actualHearingDays.findIndex((item) => item.hearingDate === hearingDate);
    }
  ...
  public static getActualDayIndexFromHearingDate(
    hearingActualsMainModel: HearingActualsMainModel,
    hearingDate: string
  ): number | undefined {
    ...
    if (hasActualsHearingDays) {
      index = hearingActualsMainModel.hearingActuals.actualHearingDays.findIndex((item) => item.hearingDate === hearingDate);
    }

    return index;
  }

  public static getPlannedDayIndexFromHearingDate(
    hearingActualsMainModel: HearingActualsMainModel,
    hearingDate: string
  ): number | undefined {
    ...
    if (hasPlannedHearingDays) {
      index = hearingActualsMainModel.hearingPlanned.plannedHearingDays.findIndex(
        (item) => ActualHearingsUtils.getDate(item.plannedStartTime) === hearingDate
      );
    }
  ...
  public static getParties(hearingActualsMainModel: HearingActualsMainModel, hearingDate: string): DisplayDayPartyModel[] {
    ...
    const actualHearingDay = hearingActualsMainModel.hearingActuals?.actualHearingDays?.find(
      (actualDay) => actualDay.hearingDate === hearingDate
    );
    const actualDayParties = actualHearingDay?.actualDayParties;

    const plannedParties =
      plannedDayIndex >= 0 ? hearingActualsMainModel.hearingPlanned.plannedHearingDays[plannedDayIndex].parties : [];

    const plannedPartyIds: string[] = plannedParties.map((plannedParty) => plannedParty.partyID);
    const actualPartiesFromPlanned: ActualDayPartyModel[] = actualDayParties?.filter((party) =>
      plannedPartyIds.some((id) => id === party.actualPartyId)
    );
    const actualDisplayModels: DisplayDayPartyModel[] = actualPartiesFromPlanned?.map((party) =>
      ActualHearingsUtils.actualToDisplayModel(party)
    );
    const plannedDisplayModels: DisplayDayPartyModel[] = plannedParties.map((party) =>
      ActualHearingsUtils.plannedToDisplayModel(party)
    );

    return actualDisplayModels?.length ? actualDisplayModels : plannedDisplayModels;
  }

  public static getPartiesCYA(hearingActualsMainModel: HearingActualsMainModel, hearingDate: string): ActualDayPartyModel[] {
    return hearingActualsMainModel.hearingActuals?.actualHearingDays?.find((actualDay) => actualDay.hearingDate === hearingDate)
      ?.actualDayParties;
  }
  ...
  public static getAttendees(hearingActualsMainModel: HearingActualsMainModel, hearingDate: string): ActualDayPartyModel[] {
    ...
    if (
      ...
      actualParties =
        actualHearingDayIndex >= 0
          ? hearingActualsMainModel.hearingActuals.actualHearingDays[actualHearingDayIndex].actualDayParties
          : [];
    }
  ...
  public static getRepresentingAttendee(
    partyId: string,
    hearingActualsMainModel: HearingActualsMainModel,
    hearingDate: string
  ): string {
  ...
  public static getPauseDateTime(day: ActualHearingDayModel, state: 'start' | 'end'): string {
    ...
    return day.pauseDateTimes && day.pauseDateTimes.length && day.pauseDateTimes[0] && day.pauseDateTimes[0].pauseStartTime
      ? day.pauseDateTimes[0][pauseTimeState]
      : null;
  }
