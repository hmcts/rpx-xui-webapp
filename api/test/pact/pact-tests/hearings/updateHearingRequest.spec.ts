...
describe('Hearings, update single hearing request', () => {
  describe('PUT /hearing/{hearingId}', () => {
    ...
    before(async () => {
      const interaction: V3Interaction = {
        states: [{ description: 'Update a hearing request' }],
        uponReceiving: 'update single hearing request',
        withRequest: {
          method: 'PUT',
    ...
    it('returns the correct response', async () => {
      return pactSetUp.provider.executeTest(async (mockServer) => {
        ...
        const { updateHearingRequest } = requireReloaded('../../../../hearings/hmc.index.ts');
        ...
        try {
          await updateHearingRequest(req, response, next);
        } catch (err) {
          throw new Error(err);
        }
