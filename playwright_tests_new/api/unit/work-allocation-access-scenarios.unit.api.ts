...
test.describe('work allocation access scenario helper', { tag: '@svc-internal' }, () => {
  ...
  test('does not treat non-access specific-access states as granted responsibilities', () => {
    const decision = queryWorkAllocationAccessByUserAndEntity(
      [
        {
          ...scenarioRecords[0],
        ...
        {
          ...scenarioRecords[0],
          assignmentId: 'assignment-alice-case-300-denied',
          caseId: '1000000000000003',
          caseName: 'Access Case 300',
  ...
  test('builds the user->entities my-access view from supported specific and challenged grants', () => {
    ...
    expect(response.cases[1]).toEqual(
      expect.objectContaining({
        case_name: 'Access Case 200',
