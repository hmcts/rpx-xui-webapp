...
Then('I see message {string} with all cases deselected', async function (message) {
  expect(await shareCasePage.getMessageDisplayedInNoCasesDisplayed(), 'Message does not match with expected').to.be.equal(
    message
  );
});
...
Then('I see last added user is marked as to be added in cases', async function () {
  expect(await shareCasePage.isLastAddedUserListedInAllCases(), 'case(s) in share case list does not have user in list').to.be
    .true;
  expect(await shareCasePage.isLastAddedUserMarkedTobeAddedInAnyCase(), 'case(s) in share case list not marked to be . added').to
    .be.true;
});
...
Then('I see a user is marked to be removed in a listed case', async function () {
  expect(await shareCasePage.isAnyUserMarkedToBeRemoved(), 'case(s) in share case list does not have a user marked to be removed')
    .to.be.true;
});
