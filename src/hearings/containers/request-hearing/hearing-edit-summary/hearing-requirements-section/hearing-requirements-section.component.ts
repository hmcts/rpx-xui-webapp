...
export class HearingRequirementsSectionComponent implements OnInit {
  ...
  public ngOnInit(): void {
    this.reasonableAdjustmentChangesRequired =
      this.hearingsService.propertiesUpdatedOnPageVisit?.afterPageVisit?.reasonableAdjustmentChangesRequired;
    this.reasonableAdjustmentChangesConfirmed =
    ...
    this.partyNamesInHMCRequestToCompare = this.hearingRequestToCompareMainModel.partyDetails
      .filter((party) => party.partyType === PartyType.IND)
      ?.map((party) => HearingsUtils.getPartyNameFormatted(party.individualDetails));
    ...
    individualParties.forEach((partyInSHV) => {
      if (this.partyIds.includes(partyInSHV.partyID)) {
        ...
        if (this.reasonableAdjustmentChangesConfirmed) {
          ...
          const baseFlagIds = reasonableAdjustments?.slice() || [];
          flagIds = CaseFlagsUtils.addLanguageFlagIfMissing(baseFlagIds, partyInSHV?.individualDetails);
        } else {
          const partyInHMC = this.partyDetails.find((partyInHMC) => partyInHMC.partyID === partyInSHV.partyID);
          if (partyInHMC) {
            const reasonableAdjustments = partyInHMC.individualDetails?.reasonableAdjustments?.filter((flagCode) =>
              flagCode?.startsWith('RA')
            );
            const baseFlagIds = reasonableAdjustments?.slice() || [];
            flagIds = CaseFlagsUtils.addLanguageFlagIfMissing(baseFlagIds, partyInHMC?.individualDetails);
          }
        }
        const flags = flagIds
          ?.map((flagId) => CaseFlagsUtils.findFlagByFlagId(this.caseFlagsRefData, flagId))
