...
class Select {
  ...
  get selectElement() {
    return this.locatorType === 'css' ? $(this.selector) : elementByXpath(this.selector);
  }

  get selectElementOptions() {
    return this.locatorType === 'css' ? $$(`${this.selector} option`) : elementsByXpath(`${this.selector}//option`);
  }
...
class GovUKRadios {
  ...
  async getOptions() {
    const labels =
      this.locatorType.toLowerCase() === 'css'
        ? $$(`${this.selector} .govuk-radios__item .govuk-radios__label`)
        : elementsByXpath(`${this.selector}//div[contains(@class,"govuk-radios__item")]//label`);
