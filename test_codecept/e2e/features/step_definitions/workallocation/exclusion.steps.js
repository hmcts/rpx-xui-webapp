...
Then('I see Add an exclusion work flow page {string} is displayed', async function (workFlowPage) {
  const workFlowPageObject = getWorkflowPageObject(workFlowPage);

  await BrowserWaits.retryWithActionCallback(async () => {
    expect(await workFlowPageObject.isDisplayed(), `${workFlowPage} work flow page not displayed`).to.be.true;
    expect(await workFlowPageObject.getHeaderText(), `${workFlowPage} work flow page header not matching`).to.include(
      workFlowPage
    );
    expect(await workFlowPageObject.getHeaderCaption(), `${workFlowPage} work flow page header caption not matching`).to.include(
      'Add an exclusion'
    );
  });
});
...
When(
  'I select Choose who the exclusion is for option {string} in add exclusion work flow',
  async function (whoTheExclusionIsForOption) {
...
When(
  'I search for caseworker from reference {string} in Find the person page of exclusion work flow',
  async function (caseworkerRef) {
...
Then(
  'I see following options returned to Select in Find person search result of exclusions work flow',
  async function (resultsDatatable) {
    ...
    await BrowserWaits.retryWithActionCallback(async () => {
      ...
      for (let i = 0; i < dataTablehashes.length; i++) {
        ...
        expect(personExpectedIsReturned, `${expectedPerson} is expected to retun in find person search is not retuned.`).to.be
          .true;
      }
    });
  }
...
Then(
  'I see caseworker from reference {string} returned to Select in Find person search result of exclusions work flow',
  async function (caseworkerRef) {
    const caseworker = global.scenarioData[caseworkerRef];
    await BrowserWaits.retryWithActionCallback(async () => {
...
When(
  'I select caseworker with reference {string} from Find person search result in exclusions work flow',
  async function (caseworkerRef) {
    const caseworker = global.scenarioData[caseworkerRef];
