...
const DEFAULT_ROLE_ACTIONS: ScenarioRoleAction[] = [
...
const getMyAccessStartDate = (record: WorkAllocationAccessScenarioRecord): string => {
  ...
  if (role.startsWith('challenged-access')) {
    return '';
  }
...
const getScenarioGrantedAccess = (record: WorkAllocationAccessScenarioRecord): boolean => {
  if (record.grantType === 'EXCLUDED') {
    return false;
  }
...
const getRoleActions = (record: WorkAllocationAccessScenarioRecord): ScenarioRoleAction[] =>
  (record.actions ?? DEFAULT_ROLE_ACTIONS).map((action) => ({ ...action }));
...
const toCaseRole = (record: WorkAllocationAccessScenarioRecord): ScenarioCaseRole => ({
  ...
  roleId: record.roleId ?? record.roleName,
...
export function queryWorkAllocationAccessByUserAndEntity(
  ...
  const responsibilities = records
    .filter((record) => record.actorId === query.actorId && record.caseId === query.caseId && getScenarioGrantedAccess(record))
    .map((record) => ({
