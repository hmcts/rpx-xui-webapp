...
describe('Header  Tabs', function () {
  ...
  it('Work allocation Tabs not present without roles "caseworker-ia-admofficer" and "caseworker-ia-caseofficer"', async function () {
    ...
    await BrowserUtil.waitForLD();
    expect(
      await headerPage().isTabPresent('Task list'),
      'Task list tab not present, displayed tabs : ' + (await headerPage().primaryNavBar.getText())
    ).to.be.false;
    expect(
      await headerPage().isTabPresent('Task manager'),
      'Task manager tab present, displayed tabs : ' + (await headerPage().primaryNavBar.getText())
    ).to.be.false;
  });
  ...
  it('Work allocation Tabs "Task list" and "Task manager" for role "caseworker-ia-caseofficer"', async function () {
    ...
    expect(
      await headerPage().isTabPresent('Task manager'),
      'Task manager tab not present, displayed tabs : ' + (await headerPage().primaryNavBar.getText())
    ).to.be.true;
    expect(
      await headerPage().isTabPresent('Task list'),
      'Task list tab not present, displayed tabs : ' + (await headerPage().primaryNavBar.getText())
  ...
  it('Work allocation Tabs for role "caseworker-ia-caseofficer", "caseworker-ia-admofficer"', async function () {
    ...
    expect(
      await headerPage().isTabPresent('Task list'),
      'Task list tab not present' + (await headerPage().primaryNavBar.getText())
    ).to.be.true;
  });
});
