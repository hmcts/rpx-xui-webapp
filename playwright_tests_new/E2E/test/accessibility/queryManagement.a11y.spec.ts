...
test.describe('Query accessibility — solicitor real session, mocked backend @accessibility @a11y', () => {
  for (const state of states) {
    test(
      ...
      {
        annotation: [
          { type: 'feature', description: 'query management' },
          { type: 'page-state', description: `solicitor raise query ${state}` },
        ],
      },
      async ({ page, caseDetailsPage, queryManagementPage }, testInfo) => {
        test.skip(
