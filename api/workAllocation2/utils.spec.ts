...
import { ASSIGN, CLAIM, CLAIM_AND_GO, COMPLETE, GO, Permission, REASSIGN, RELEASE } from './constants/actions';
...
describe('workAllocation.utils', () => {
  ...
  describe('getTaskActionsByPermissions', () => {

    it('should get correct actions for my tasks for certain permissions', () => {
      expect(getTaskActionsByPermissions('MyTasks', [Permission.CANCEL, Permission.MANAGE]))
        .to.deep.equal([REASSIGN, RELEASE, GO]);
      expect(getTaskActionsByPermissions('MyTasks', [Permission.EXECUTE])).to.deep.equal([]);
      expect(getTaskActionsByPermissions('MyTasks', [Permission.MANAGE, Permission.CANCEL]))
        .to.deep.equal([REASSIGN, RELEASE, GO]);
    });

    it('should get correct actions for available tasks for certain permissions', () => {
      expect(getTaskActionsByPermissions('AvailableTasks', [Permission.CANCEL, Permission.MANAGE]))
        .to.deep.equal([CLAIM, CLAIM_AND_GO]);
      expect(getTaskActionsByPermissions('AvailableTasks', [Permission.EXECUTE])).to.deep.equal([]);
      expect(getTaskActionsByPermissions('AvailableTasks', [Permission.MANAGE, Permission.CANCEL]))
        .to.deep.equal([CLAIM, CLAIM_AND_GO]);
    });

    it('should get correct actions for all work tasks for certain permissions', () => {
      expect(getTaskActionsByPermissions('AllWorkAssigned', [Permission.MANAGE]))
        .to.deep.equal([REASSIGN, RELEASE, GO]);
      expect(getTaskActionsByPermissions('AllWorkAssigned', [Permission.EXECUTE]))
        .to.deep.equal([COMPLETE]);
      expect(getTaskActionsByPermissions('AllWorkUnassigned', [Permission.MANAGE, Permission.EXECUTE]))
