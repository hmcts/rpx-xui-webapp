#!/usr/bin/env node
/**
 * Moves the Playwright API report into the Jenkins publish folder so only one
 * report and trace tree is retained. No-op if the report is missing.
 */
const fs = require('fs');
...
const sourceResults = path.resolve('functional-output', 'tests', 'playwright-api', 'test-results');
const targetResults = path.join(targetRoot, 'test-results');
...
try {
  ...
  // Move the report and results so Jenkins retains one canonical tree.
  fs.rmSync(target, { recursive: true, force: true });
  fs.renameSync(source, target);
  if (fs.existsSync(sourceResults)) {
    fs.rmSync(targetResults, { recursive: true, force: true });
    fs.renameSync(sourceResults, targetResults);
  }
  if (existingJunit) {
  ...
  enhanceGeneratedReport(target, []);

  normalizeApiOdhinFilename(target);

  const reportFolders = [target];
