...
import { ConfirmExclusionAction, ExclusionActionTypes, SpecificAccessActionTypes } from '../actions';
...
export class SpecificAccessEffects {
  ...
  @Effect() public confirmSpecificAccess$ = this.actions$
    .pipe(
      ...
      mergeMap(
        (data) => this.roleExclusionsService.confirmExclusion(data.payload)
          .pipe(
            ...
            catchError(error => {
              return SpecificAccessEffects.handleError(error, SpecificAccessActionTypes.CHANGE_NAVIGATION);
            }
            )
          )
      )
