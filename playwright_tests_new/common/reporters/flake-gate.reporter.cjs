class FlakeGateReporter {
  constructor() {
    ...
    this.maxFlakyTests = Number.isFinite(Number(process.env.PW_MAX_FLAKY_TESTS)) ? Number(process.env.PW_MAX_FLAKY_TESTS) : 0;
    this.maxFlakyRate = Number.isFinite(Number(process.env.PW_MAX_FLAKY_RATE)) ? Number(process.env.PW_MAX_FLAKY_RATE) : 0;
    this.reportOnly = ['1', 'true', 'yes', 'on', 'report-only'].includes(
      String(process.env.PW_FLAKE_GATE_REPORT_ONLY ?? process.env.PW_FLAKE_GATE_MODE ?? '').toLowerCase()
    );
  }
  ...
  onEnd() {
    ...
    const shouldFail = !this.reportOnly && (exceededFlakyCount || exceededFlakyRate);
