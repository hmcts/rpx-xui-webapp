...
const pactSetUp = new PactV3TestSetup({ provider: 'am_orgRoleMapping_refresh', port: 8000 });
...
describe('get /am/role-mapping/judicial/refresh', () => {
  ...
  before(async () => {
    const interaction = {
      states: [{ description: 'A refresh request is received with a valid userId passed' }],
