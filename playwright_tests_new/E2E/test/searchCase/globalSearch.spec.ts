...
test.describe('IDAM login using credentials for Global Search', { tag: ['@e2e', '@e2e-search-case'] }, () => {
  ...
  test("Global Search (Partial) - using '*' wildcard on case number", async ({ globalSearchPage, tableUtils, page }) => {
    const availableCaseReference = await resolveCaseReferenceFromGlobalSearch(page, PUBLIC_LAW_CASE_REFERENCE_OPTIONS);
