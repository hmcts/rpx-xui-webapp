...
class RoleAssignments {
  ...
  getQueryFor(response) {
    ...
    if (
      bodyKeys.includes('queryRequests') &&
    ...
    if (bodyKeys.includes('queryRequests') && reqBody.queryRequests[0].attributes.caseId) {
      return 'CASE_ROLES';
    }
  }
  ...
  setCTSCCaseRoles() {
    ...
    for (const roleType of ctscRoles) {
      const iadamId = rdCaseworkerRefApi.iacCTSCUsers[ctscuserCtr].id;
      ctscuserCtr++;
      this.caseRoles.push(
        roleTemplates.caseRole({
          actorId: iadamId,
