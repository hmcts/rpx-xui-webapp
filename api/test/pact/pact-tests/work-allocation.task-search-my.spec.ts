...
import { SORTABLE_FIELDS } from '../constants/work-allocation/tasks.spec';
import { CASEWORKERS, toAssignee } from './../constants/work-allocation/caseworkers.spec';
import { filterByAssignee, sortTasks, TASKS_ARRAY } from './../constants/work-allocation/tasks.spec';
...
describe('Work Allocation API', () => {
  ...
  before(async () => {
    ...
    provider = new Pact({
      consumer: 'xui_work_allocation_task_search_my',
  ...
  // John Smith is a proxy for the current user.
  const assigneeName = toAssignee(CASEWORKERS.JOHN_SMITH).userName;
  const operator = 'my';
  ...
  for (const key of SORTABLE_FIELDS) {
    ...
    for (const order of ['ascending', 'descending']) {
      ...
      const tasks = sortTasks(filterByAssignee(TASKS_ARRAY, assigneeName), key, order);
