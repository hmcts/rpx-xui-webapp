    ...
    return hearingState$.pipe(
      map((state) => {
        ...
        if (objAHearingWindow.dateRangeStart && objBHearingWindow.dateRangeStart) {
          const isStartDatesUpdated =
            moment(objAHearingWindow.dateRangeStart).format(HearingDateEnum.DefaultFormat) !==
            moment(objBHearingWindow.dateRangeStart).format(HearingDateEnum.DefaultFormat);

          let isEndDatesUpdated = false;
          if (objAHearingWindow.hasOwnProperty('dateRangeEnd') || objBHearingWindow.dateRangeEnd) {
            const dateObjA = objAHearingWindow.hasOwnProperty('dateRangeEnd')
              ? moment(objAHearingWindow.dateRangeEnd).format(HearingDateEnum.DefaultFormat)
              : null;
            const dateObjB = objBHearingWindow.dateRangeEnd
              ? moment(objBHearingWindow.dateRangeEnd).format(HearingDateEnum.DefaultFormat)
              : null;
          ...
          return (
            moment(objAHearingWindow.firstDateTimeMustBe).format(HearingDateEnum.DefaultFormat) !==
            moment(objBHearingWindow.firstDateTimeMustBe).format(HearingDateEnum.DefaultFormat)
          );
        }
