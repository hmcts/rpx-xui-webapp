...
describe('Work Allocation API', () => {
  ...
  for (const cwType of ['all', 'unassigned']) {
    let caseworkerNames = [];
    let baseTasks = [ ...TASKS_ARRAY ];
    if (cwType === 'all') {
      caseworkerNames = ALL_CASEWORKERS.map(cw => `${cw.firstName} ${cw.lastName}`);
    } else {
      baseTasks = [ ...getUnassignedTasks() ];
    }
    ...
    for (const key of SORTABLE_FIELDS) {
      for (const locationKey in LOCATIONS) {
        ...
        if (location === LOCATIONS.ALL) {
          values = LOCATIONS_ARRAY.map(l => l.locationName);
        }
        ...
        for (const order of ['ascending', 'descending']) {
          ...
          describe(`when requested to search for ${cwType} tasks at ${location.locationName} in ${order} order of ${key}`, () => {
            before(() =>
              provider.addInteraction({
                state: 'a list of appropriate tasks are returned',
                uponReceiving: `a valid request to search for ${cwType} tasks at ${location.locationName} in ${order} order of ${key}`,
                withRequest: {
                  method: 'POST',
                  path: '/task',
                  body: request
