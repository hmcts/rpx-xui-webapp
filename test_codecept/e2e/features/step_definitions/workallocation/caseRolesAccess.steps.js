...
Then(
  'I validate for role category {string} case roles table displayed status is {string} in case roles and access page',
  async function (roleCategory, displayStatus) {
...
Then(
  'I validate for role category {string} case roles no data message displayed status is {string} in case roles and access page',
  async function (roleCategory, displayStatus) {
    const role = roleCategory.toLowerCase().replace(' ', '');
    const expectedIsDisplayed = displayStatus.toLowerCase().includes('true');
    if (role.includes('judicia')) {
      expect(await caseRolesAndAccessPage.judicialRolesAccessTable.isNoDataSummaryMessageDisplayed()).to.equal(
        expectedIsDisplayed
      );
    } else if (role.includes('legalops')) {
      expect(await caseRolesAndAccessPage.legalOpsRolesAccessTable.isNoDataSummaryMessageDisplayed()).to.equal(
        expectedIsDisplayed
      );
    } else if (role.includes('exclusion')) {
...
Then(
  'I validate for role category {string} in case roles and access message displayed as {string}',
  async function (roleCategory, message) {
    const role = roleCategory.toLowerCase().replace(' ', '');
    if (role.includes('judicia')) {
...
Then(
  'I validate case roles table has headers for role category {string} in case roles and access page',
  async function (roleCategory, headerColumnsDatatab) {
    const headerNameHashes = headerColumnsDatatab.parse().hashes();
    await BrowserWaits.retryWithActionCallback(async () => {
      for (const headerHash of headerNameHashes) {
        expect(
          await caseRolesAndAccessPage.isTableColumnDisplayedForAccessRoleType(roleCategory, headerHash.headerName),
          `${headerHash.headerName} column is not displayedin case role table for role ${roleCategory}`
        ).to.be.true;
      }
    });
  }
...
Then('I validate case roles table for role category {string} has data', async function (roleCategory, rowsDatatabale) {
  ...
  for (const rowHash of rowHashes) {
    ...
    for (const headerName of headerNames) {
      ...
      expect(
        await caseRolesAndAccessPage.getTableColumnValueForAccessRoleType(roleCategory, rowNum, headerName),
        `${headerName} column value does not match ${roleCategory}`
...
Then(
  'I validate case roles {string} link displayed status is {string} for category {string}',
  async function (linkText, isDisplayedAsString, roleCategory) {
    ...
    for (let i = 1; i <= rolesCount; i++) {
      await BrowserWaits.retryWithActionCallback(async () => {
        expect(
          await caseRolesTable.isLinkWithTextPresentAtRow(i, linkText),
          `${linkText} at row ${i} expected to display ${expectedDisplayedStatus} check failed.`
        ).to.equal(expectedDisplayedStatus);
      });
    }
  }
);
...
When(
  'I click manage link for role category {string} at row {int} in Roles and access page',
  async function (roleCategory, atRow) {
    const caseRolesTable = caseRolesAndAccessPage.getTableForRoleACcessType(roleCategory);
...
Then(
  'I validate actions row for role category {string} at row {int} is {string} in Roles and access page',
  async function (roleCategory, atRow, isDisplayedString) {
...
Then(
  'I validate actions row for role category {string} has action links in Roles and access page',
  async function (roleCategory, actionLinksdatatable) {
...
When(
  'I click action row link {string} for role category {string} in Roles and access page',
  async function (actionLabel, roleCategory) {
    const caseRolesTable = caseRolesAndAccessPage.getTableForRoleACcessType(roleCategory);
