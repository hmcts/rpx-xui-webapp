...
When(
  'I select search criteria jurisdiction {string} case type {string} state {string} in case list page',
...
When(
  'I select search criteria jurisdiction {string} case type {string} state {string} in case list page and click apply',
  async function (jurisdiction, caseType, state) {
    await BrowserWaits.retryWithActionCallback(async () => {
      try {
        await caseListPage.selectJurisdiction(jurisdiction);
...
Then('I see case list table header has Select all checkbox column', async function () {
  expect(await caseListPage.isSelectAllColumnDisplayed(), 'Case list selection Select all column/checkbox not present ').to.be
    .true;
});
...
Then('I see case list table each case row has checkbox column', async function () {
  expect(await caseListPage.isSelectCheckboxDisplayedForAllCases(), 'Case select checkbox not displayed for all acses').to.be
    .true;
});
...
Then('I see case list table has case at row {int} selected', async function (rowNum) {
  expect(await caseListPage.isCaseSelectCheckboxSelected(rowNum), 'Case select checkbox not selected for case at row ' + rowNum)
    .to.be.true;
});

Then('I see case list table has case at row {int} un-selected', async function (rowNum) {
  expect(await caseListPage.isCaseSelectCheckboxSelected(rowNum), 'Case select checkbox still selected for case at row ' + rowNum)
    .to.be.false;
});
...
Then('I see case list table has {int} rows selected', async function (selectedCasesCount) {
  expect(
    await caseListPage.getCountOfCasesSelectedInPage(),
    'Selected cases count does not match expected count' + selectedCasesCount
  ).to.be.equal(selectedCasesCount);
});
...
Then('I see case list selection feature {string} available', async function (featureCondition) {
  ...
  if (featureCondition.toLowerCase() === 'is') {
    ...
    expect(await caseListPage.tableHeaderSelectAllInput.isPresent(), 'Table header Select All check box should not be  present')
      .to.be.false;
