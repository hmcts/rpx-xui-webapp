...
export async function assignCases(req: EnhancedRequest, res: Response, next: NextFunction) {
  ...
  // call share case api
  await doShareCase(req, next, shareCases, updatedSharedCases, errorMessages)
...
async function doShareCase(req: EnhancedRequest, next: NextFunction, shareCases: SharedCase[],
  ...
  for (const aCase of shareCases) {
    ...
    if (aCase.pendingShares) {
      for (const pendingShare of aCase.pendingShares) {
        ...
        try {
          const {status}: { status: number } = await handlePost(path, payload, req, next)
          if (status === 201) {
...
// @ts-ignore
async function doUnshareCase(req: EnhancedRequest,
                             next: NextFunction,
